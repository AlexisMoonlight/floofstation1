...
namespace Content.MapRenderer.Painters
{
    public sealed class MapPainter
    {
        ...
        public static async IAsyncEnumerable<RenderedGridImage<Rgba32>> PaintGrind()
        {
            var stopwatch = new Stopwatch();
            stopwatch.Start();
            ...
            await server.WaitPost(() =>
            {
                ...
                var loadOptions = new MapLoadOptions {StoreMapUids = true,DoMapInit = true};
                var mapEntityId= sEntityManager.System<MapSystem>().CreateMap(out var mapId);
                sEntityManager.System<MapLoaderSystem>().TryLoad(mapId, "/Maps/core.yml", out _, loadOptions);
                grids = sMapManager.GetAllGrids(mapId).ToArray();
